...
description: |
  Sets up GitHub Actions CI/CD workflows for WordPress plugins — coding standards (WPCS/PHPCS), PHP/JS/CSS linting, PHPUnit testing, static analysis (PHPStan), Composer security scanning, WordPress Playground PR previews, and automated deployment to WordPress.org. Use this skill whenever the user asks to add CI/CD, automated testing, linting, or deployment workflows to a WordPress plugin repository. Also trigger when the user mentions "GitHub Actions" alongside WordPress, "automate my plugin", "deploy to WordPress.org", "add PHPCS", "add linting", "WordPress coding standards check", "plugin CI pipeline", "PR preview", "Playground preview", or "plugin release workflow". If the user is inside a WordPress plugin repository and asks about quality checks, automated testing, or deployment, use this skill.
---
